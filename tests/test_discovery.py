...
def test_fetch_potential_pains(discovery_module, mock_scraper):
    ...
    posts = discovery_module.fetch_potential_pains(["test-sub"], limit_per_target=10)

    assert len(posts) == 1
    assert posts[0].title == "Pain 1"
    mock_scraper.scrape.assert_called_once_with(target="test-sub", limit=10)
...
def test_analyze_pain_intensity(discovery_module, mock_llm):
    post = ScrapedPost(
        id="1",
        source="reddit",
        title="I hate manual data entry",
        body="It takes hours.",
        author="user1",
        url="url1",
        upvotes=10,
